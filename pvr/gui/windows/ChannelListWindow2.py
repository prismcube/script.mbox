...
class ChannelListWindow(BaseWindow):
	...
	def onAction(self, action):
		...
		if id == Action.ACTION_PREVIOUS_MENU:
			...
			if focusId == self.ctrlListCHList.getId() :
				...
				print '<<<<< test youn #1: action ID[%s]' % id			
	...
	def onEvent(self, event):
		...
		print 'event #0 %d' %(xbmcgui.getCurrentWindowId( ))

		print 'event #0 %d' %self.winId
		...
		if xbmcgui.getCurrentWindowId( ) == self.winId :
			...
			print 'show screen is another windows page[%d]' %self.winId
