...
class FirstInstallation( SettingWindow ) :
	...
	def SetListControl( self, aStep ) :
		...
		if self.mStepNum == E_STEP_SELECT_LANGUAGE :
			...
			self.AddInputControl( E_Input01, MR_LANG( 'Menu Language' ), self.mMenuLanguageList[ ElisPropertyEnum( 'Language', self.mCommander ).GetPropIndex( ) ], MR_LANG( 'Select the satellite(s) you wish to search from' ) )
			self.AddInputControl( E_Input02, MR_LANG( 'Audio Language' ), self.mAudioLanguageList[ ElisPropertyEnum( 'Audio Language', self.mCommander ).GetPropIndex( ) ], MR_LANG( 'Select the satellite(s) you wish to search from' ) )
