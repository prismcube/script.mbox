...
class DialogMoveAntenna( BaseDialog ) :
	...
	def onInit( self ) :
		...
		icon = [ 'OSDRewindNF.png', 'OSDPlayNF_Rotated.png', 'OSDPlayPauseNF.png', 'OSDPlayNF.png', 'OSDForwardNF.png' ]
	...
	def onClick( self, aControlId ) :
		if aControlId == DIALOG_BUTTON_CLOSE_ID :
			...
			if selectedIndex == E_MOVE_EAST :
				...
				if ret :
					dialog.SetDialogProperty(  MR_LANG( 'Reset Complete' ),  MR_LANG( 'The limit is reset to 0' ) )
				else :
				...
				dialog = DiaMgr.GetInstance( ).GetDialog( DiaMgr.DIALOG_ID_POPUP_OK )
				if ret :
					dialog.SetDialogProperty(  MR_LANG( 'Save Complete' ),  MR_LANG( 'That position is saved' ) )
				else :
