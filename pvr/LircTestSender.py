...
class _VKey(object):
	def __init__(self):
		...
		for i in range(len(KeyCodeStr)) :
			self.mHashKey[KeyCodeStr[i][0]] = KeyCodeStr[i][1]

def manual_operation( sock ) :
	isInput = True
	key = GetInstanceHash()
	while (1) :
		inputKey = raw_input('input (\'help\' or \'?\' or Enter is Continue): ')

		if inputKey == '' :
			isInput = False
			break

		if inputKey.lower() == 'help' or inputKey == '?' :
			for item in KeyCodeStr :
				print item[0]
			continue

		vkey = key.mHashKey.get(inputKey)
		if not vkey :
			print 'unknown key[%s]'% inputKey
			continue

		#print vkey
		msg = struct.pack("3i",*[1, vkey, 0])
		print 'send[%s] size[%s] '% (vkey, len(msg) )
		sock.send( msg )
		time.sleep(1)
...
def delay_sleep( aSleep ) :
	inputKey = False
	for i in range(aSleep) :
