...
class DownloadFile( object ) :
	...
	def __startHttpResume__( self, restart = None, callBack = None ) :
		...
		if self.getType() == 'ftp' :
			if not self.__offsetFtpResume__( urllib2Obj, f, callBack = callBack ) :
				return

		self.__downloadFile__( urllib2Obj, f, callBack = callBack )


	def __offsetFtpResume__( self, urlObj, fileObj, callBack = None ) :
		ret = True
		self.cur = 0
		curSize = self.getLocalFileSize( )
		#print '-------resumSize[%s] totSize[%s] cur[%s]'% ( curSize, totSize, self.cur )
		...
		while self.isContinue :
			...
			if ( self.cur + 8192 ) <= curSize :
				self.cur += 8192
			else :
				rbyte = 8192 - ( curSize - self.cur )
				fileObj.write( data[rbyte:] )
				self.cur += rbyte
				print 'offset rbyte[%s]'% rbyte
				break
	...
	def __startFtpResume__( self, callBack = None, restart = None ) :
		...
		if callBack :
			callBack( cursize = self.getLocalFileSize( ) )
		...
		thread = threading.Timer( 0.5, self.getCallback, [callBack,ftper,f] )
		thread.start( )
	...
	def getCallback( self, callBack = None, ftper = None, fSock = None ) :
		while self.isContinue == True :
			time.sleep( 0.1 )
			if callBack :
				cur = self.getLocalFileSize( )
				callBack( cursize = cur )

		if self.isContinue == False and ftper :
			ftper.abort( )
	...
	def resume(self, callBack=None):
		...
		if type == 'http' :
			...
			#self.__startFtpResume__( callBack = callBack )
			self.__startHttpResume__( callBack = callBack )


	def abort( self, value = False ) :
		self.isContinue = not value
